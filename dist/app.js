...
var $author$project$PhotoFolders$initialModel = {
	...
	G: {L: true, ai: 'Loading...', al: _List_Nil, S: _List_Nil},
...
var $author$project$PhotoFolders$photosDecoder = function () {
	var jsonPhotoDecoder = A3(
	...
	var finishPhoto = function (_v0) {
		var url = _v0.a;
		var _v1 = _v0.b;
		var title = _v1.a;
		var size = _v1.b;
		var relatedUrls = _v1.c;
		return _Utils_Tuple2(
			url,
			{an: relatedUrls, ap: size, bU: title, au: url});
	};
	var fromPairs = A2(
		$elm$core$Basics$composeR,
		$elm$core$List$map(finishPhoto),
		$elm$core$Dict$fromList);
	return A2(
		$elm$json$Json$Decode$map,
		fromPairs,
		$elm$json$Json$Decode$keyValuePairs(jsonPhotoDecoder));
}();
...
var $author$project$PhotoFolders$modelDecoder = A3(
	...
	F2(
		function (photos, root) {
			return {aa: photos, G: root, z: $elm$core$Maybe$Nothing};
		}),
		...
		$elm$http$Http$get(
			{
				...
				au: 'http://elm-in-action.com/folders/list'
			}));
};
...
var $author$project$PhotoGallery$Photo = F3(
	function (url, size, title) {
		return {ap: size, bU: title, au: url};
	});
...
var $author$project$PhotoGallery$initialCmd = $elm$http$Http$get(
	{
		...
		au: 'http://elm-in-action.com/photos/list.json'
	});
var $author$project$PhotoGallery$Loading = {$: 0};
var $author$project$PhotoGallery$Medium = 1;
var $author$project$PhotoGallery$initialModel = {J: '', Y: 1, M: 5, O: 5, R: 5, q: $author$project$PhotoGallery$Loading};
	...
	function (model, _v0) {
		...
		return _Utils_Tuple2(
			_Utils_update(
				model,
				{
					n: $author$project$Main$GalleryPage(folders)
				}),
...
var $author$project$Main$init = F3(
	function (version, url, key) {
		return A2(
			...
			url,
			{ag: key, n: $author$project$Main$NotFound, av: version});
	});
	...
	switch (_v0.$) {
		case 1:
			...
			var filters = _List_fromArray(
				[
					{X: model.M / 11, ai: 'Hue'},
					{X: model.R / 11, ai: 'Ripple'},
					{X: model.O / 11, ai: 'Noise'}
				]);
			return _Utils_Tuple2(
				model,
...
var $author$project$PhotoFolders$viewFolder = F2(
	function (path, _v0) {
		...
		var folderLabel = A2(
			...
			_List_fromArray(
				[
					$elm$html$Html$text(folder.ai)
				]));
		if (folder.L) {
			var contents = A2(
				$elm$core$List$append,
				A2($elm$core$List$indexedMap, viewSubfolder, folder.S),
				A2($elm$core$List$map, $author$project$PhotoFolders$viewPhoto, folder.al));
			return A2(
				$elm$html$Html$div,
				_List_fromArray(
					[
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$elm$html$Html$Attributes$src($author$project$PhotoFolders$urlPrefix + ('photos/' + (photo.au + '/full')))
					]),
				_List_Nil),
				A2(
				$elm$html$Html$span,
				...
				_List_fromArray(
					[
						$elm$html$Html$text(
						$elm$core$String$fromInt(photo.ap) + 'KB')
					])),
				A2(
				$elm$html$Html$h3,
				_List_Nil,
				...
				A2($elm$core$List$map, $author$project$PhotoFolders$viewRelatedPhoto, photo.an))
			]));
};
var $author$project$PhotoFolders$view = function (model) {
	var photoByUrl = function (url) {
		return A2($elm$core$Dict$get, url, model.aa);
	};
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					thumb.bU + (' [' + ($elm$core$String$fromInt(thumb.ap) + ' KB]'))),
	...
	function (photos, selectedUrl, model) {
		return _List_fromArray(
			[
				...
				_List_fromArray(
					[
						A3($author$project$PhotoGallery$viewFilter, $author$project$PhotoGallery$SlidHue, 'Hue', model.M),
						A3($author$project$PhotoGallery$viewFilter, $author$project$PhotoGallery$SlidRipple, 'Ripple', model.R),
						A3($author$project$PhotoGallery$viewFilter, $author$project$PhotoGallery$SlidNoise, 'Noise', model.O)
					])),
				A2(
				$elm$html$Html$h3,
				_List_Nil,
				...
				_List_fromArray(
					[
						...
						$author$project$PhotoGallery$sizeToString(model.Y))
					]),
				A2(
					$elm$core$List$map,
